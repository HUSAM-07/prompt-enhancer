...
# Configure page
st.set_page_config(
    page_title="Claude Prompt Enhancement Studio",
    page_icon="🚀",
    layout="wide",
...
class PromptImprovementEngine:
    ...
    def __init__(self):
        self.improvement_prompt_template = """You are an expert prompt engineer specializing in optimizing prompts for Claude and other LLMs.
...
- [Suggest helpful examples]
    ...
    def improve_prompt(self, 
        ...
        """Use Claude to automatically improve a prompt"""
...
Format as: <chain_of_thought>[Your reasoning here]</chain_of_thought>"""
    ...
    def enrich_example(self, example: Example) -> Example:
        ...
        # Call Claude to generate chain of thought
        headers = {
            "Authorization": f"Bearer {OPENAI_API_KEY}",
            "Content-Type": "application/json"
        }
...
def main():
    st.title("🚀 Claude Prompt Enhancement Studio")
    st.markdown("Advanced prompt engineering with automatic improvement, example management, and evaluation")
    ...
    with st.sidebar:
        ...
        st.markdown("""
        1. **Enter your prompt** in the main area
        2. **Add examples** to improve accuracy
        3. **Auto-improve** using Claude's expertise
        4. **Evaluate** with test cases
        ...
        with col1:
            ...
            with col_btn1:
                if st.button("🚀 Auto-Improve Prompt", type="primary", use_container_width=True):
                    if original_prompt:
                        with st.spinner("Claude is improving your prompt..."):
